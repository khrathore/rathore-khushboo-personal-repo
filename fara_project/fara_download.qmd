...
egypt_docs <- filtered_docs %>% 
  ...
  inner_join(docs_country, by = "registration_number") %>% 
  filter(!str_detect(url, "Available")) %>% 
  mutate(foreign_principal_country = foreign_principal_country.y) %>% 
  select(-foreign_principal_country.x, -foreign_principal_country.y)
...
=======
walk(c(1:nrow(egypt_analysis)), get_pdfs)
>>>>>>> Stashed changes
