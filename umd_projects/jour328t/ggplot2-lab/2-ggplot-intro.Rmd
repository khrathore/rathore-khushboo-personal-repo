...
```{r}
custom_data <- data %>% 
  filter(name == "Khushboo")

custom_data %>% 
  ggplot(aes(x = year, y = n)) + 
  geom_col(fill = "pink", alpha = .8) +
  labs(title = "The occurences of Khushboo", subtitle = "Up and down, possibly according to the astrological charts", caption = "Source: Social Security Administration\nby Olga Pierce") +
  theme_minimal() +
  theme(axis.title = element_blank(),
        axis.text.y = element_blank(),
        panel.grid.minor = element_blank()) +
...
```{r}
line_graphdata <- data %>% 
  filter(year >= 1990 & year <= 2015 & sex == "F") %>% 
  filter(name %in% c("Bethany", "Josephine", "Kennedy", "Alice"))

line_graphdata %>% ggplot(aes(x = year, y = prop, color = name, fill = name)) +
  geom_density(stat = "identity", alpha = .1) +
  scale_y_continuous(labels = scales::percent) +
  scale_x_continuous(breaks = seq(1990,2015,5)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Just some random names", subtitle = "Names as a percentage of all births")
```
